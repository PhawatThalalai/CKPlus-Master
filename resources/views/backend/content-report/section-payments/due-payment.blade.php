...
<br/>
	...
	<span>ชำระโดย : <u>{{ @$CHQMas->CHMasPAYTYP->PAYDESC }}</u></span>
	...
	<table border="0" style="width: 400px;line-height: 80%;" align="center">
		<tr>
			<td align="left"></td>
			<td class="spac-ms"><b>ชำระค่างวด</b></td>
		...
		@if ($contcus->CODLOAN == 1)
			<tr>
				<td align="center"></td>
				<td class="spac-ms"><b>ชำระเงินต้น</b></td>
				<td align="right" class="spac-md">{{ number_format(@$CHQTran->PAYAMT_N, 2) }}</td>
				<td align="right" class="spac-md">บาท</td>
			</tr>
			<tr>
				<td align="center"></td>
				<td class="spac-ms"><b>ชำระดอกเบี้ย</b></td>
		...
		<tr>
			<td align="center"></td>
			<td class="spac-ms"><b>ชำระดอกเบี้ยผิดนัดชำระ</b></td>
		...
		@if (@$CHQTran->DSCINT != 0)
			<tr>{{-- >0 --}}
				<td align="center"></td>
				<td class="spac-ms"><b>ส่วนลดดอกเบี้ยผิดนัดชำระ</b></td>
		...
		<tr>
			<td align="center"></td>
			<td class="spac-ms"><b>ชำระค่าทวงถาม</b></td>
		...
		@if (@$CHQTran->DSCPAYFL != 0)
			<tr>{{-- >0 --}}
				<td align="center"></td>
				<td class="spac-ms"><b>ส่วนลดค่าทวงถาม</b></td>
		...
		<tr>
			<td align="center"></td>
			<td class="spac-ms"><b>รวมชำระ</b></td>
			<td align="right" class="spac-md">{{ number_format(@$CHQTran->NETPAY, 2) }}</td>
			<td align="right" class="spac-md">บาท</td>
		</tr>
		<tr>
			<td align="center"></td>
			<td align="right" class="spac-ms " colspan="3"><span class="underlined">( {{ IntconvertThai(@$CHQMas->CHQAMT) }} )</span></td>

		</tr>
		@if (@$CHQTran->DISCT != 0)
			<tr>{{-- >0 --}}
				<td align="center"></td>
				<td class="spac-ms"><b>ส่วนลด</b></td>
				<td align="right" class="spac-md">{{ number_format(@$CHQTran->DISCT, 2) }}</td>
				<td align="right" class="spac-md">บาท</td>
			</tr>
		@endif
		{{-- <tr>
					<td align="left" colspan="2"><b>ยอดเงินชำระมาแล้ว ณ วันที่รับชำระ</b></td>
					<td align="right" class="spac-md">{{ number_format(@$item->PAYAMT, 2) }}</td>
					<td align="right" class="spac-md">บาท</td>
				</tr> --}}
		{{-- <tr>
			<td align="left" colspan="2"><b>ยอดคงเหลือ</b></td>
			<td align="right" class="spac-md">{{ number_format(@$item->PAYAMT, 2) }}</td>
			<td align="right" class="spac-md">บาท</td>
		</tr> --}}
	</table>
	...
	{{-- <table border="0" width="100%" style="margin-top: 10px; border: 1px solid; border-color: #dfdfdf">
		<thead>
			<tr style="line-height: 130%; background-color: #D9E2F3; font-size: 17px">
				<th align="left" class="spac-ms" colspan="3">
					<span><b> สำหรับช่องทางชำระเงิน</b></span>
					<span><b> (FOR PAYMENT CHANELS)</b></span>
				</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				...
				<td align="left" style=" padding-top: 5px;">
					<span style="">
						<img height="40" width="350" src="{{ getcwd() . '/cache_barcode/' . $_namefile[0] . '.png' }}" alt="barcode" />
						<div>{{ $_namefile[2] }}</div>
					</span>
				</td>
				<td align="right" style=" padding-top: 5px;">
					<img height="80" src="{{ getcwd() . '/cache_barcode/' . $_namefile[1] . '.svg' }}" alt="qrcode">
				</td>
			</tr>
		</tbody>
	</table> --}}
