...
<body>
	...
	<table border="1" width="100%" style="margin: 0px;">
		<tr style="line-height: 80%">
			...
			<td width="50%" align="right">
				...
				<P class="rs-margin m-0">พิมพ์ : {{ date('d-m-Y') }}</P>
				<P class="rs-margin">เลขที่เอกสาร : {{ @$data->DOCNO }}</P>
			</td>
		</tr>
	</table>
				...
				@if (@$contcus->id_rateType == 'land')
					<table border="0">
						<tr>
							...
							<td width="35%">{{ @$contasset->IndenAssetToDataOwner->DataAssetToLandType->nametype_car }}</td>
							<td width="17%">เลขที่โฉนด :</td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Land_Id }}</td>
						</tr>
						<tr>
							<td>เลขที่ดิน :</td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Land_ParcelNumber }}</td>
							<td>เล่ม/หน้า :</td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Land_Book . '/' . @$contasset->IndenAssetToDataOwner->Land_BookPage }}</td>
						</tr>
						<tr>
							<td>อำเภอ :</td>
							<td>{{ Str::limit(@$contasset->IndenAssetToDataOwner->Land_District, 25) }}</td>
							<td>จังหวัด :</td>
							<td>{{ Str::limit(@$contasset->IndenAssetToDataOwner->Land_Province, 25) }}</td>
						</tr>
					</table>
				@else
					<table border="0">
						<tr>
							...
							<td width="45%">{{ @$contasset->IndenAssetToDataOwner->AssetToCarType->nametype_car }}</td>
							<td width="16%">ยี่ห้อ :</td>
							<td>{{ @$contasset->IndenAssetToDataOwner->AssetToCarBrand->Brand_car }}</td>
						</tr>
						<tr>
							<td width="16%">กลุ่ม :</td>
							<td>{{ Str::limit(@$contasset->IndenAssetToDataOwner->AssetToCarGroup->Group_car, 25) }}</td>
							<td width="16%">ปี :</td>
							<td>{{ @$contasset->IndenAssetToDataOwner->AssetToCarYear->Year_car }}</td>
						</tr>
						<tr>
							<td width="16%">รุ่น :</td>
							<td>{{ Str::limit(@$contasset->IndenAssetToDataOwner->AssetToCarModel->Model_car, 25) }}</td>
							<td width="16%">ทะเบียน :</td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Vehicle_NewLicense == null ? @$contasset->IndenAssetToDataOwner->Vehicle_OldLicense : @$contasset->IndenAssetToDataOwner->Vehicle_NewLicense }}</td>
						</tr>
					</table>
				@endif
			</td>
	...
	<span>ชำระโดย : {{ @$CHQMas->CHMasPAYTYP->PAYDESC }}</span>
			...
			<td colspan="2" class="spac-ms">
				@if ($contcus->id_rateType == 'land')
					<table border="0">
						<tr>
							...
							<td>{{ @$contasset->IndenAssetToDataOwner->DataAssetToLandType->nametype_car }}</td>
							<td width="13%"><b>เลขที่โฉนด : </b></td>
							<td width="15%">{{ @$contasset->IndenAssetToDataOwner->Land_Id }}</td>
							<td width="11%"><b>เลขที่ดิน : </b></td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Land_ParcelNumber }}</td>
						</tr>
						<tr style="line-height: 90%;">
							<td><b>ระวาง : </b></td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Land_SheetNumber }}</td>
							<td><b>หน้าสำรวจ : </b></td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Land_TambonNumber }}</td>
							<td><b>เล่ม/หน้า : </b></td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Land_Book . '/' . @$contasset->IndenAssetToDataOwner->Land_BookPage }}</td>
						</tr>
						<tr style="line-height: 90%;">
							<td><b>เนื้อที่ : </b></td>
							<td>
								{{ (empty(@$contasset->IndenAssetToDataOwner->Land_SizeRai) ? '-' : @$contasset->IndenAssetToDataOwner->Land_SizeRai) }}  
								{{ (empty(@$contasset->IndenAssetToDataOwner->Land_SizeNgan) ? '-' : @$contasset->IndenAssetToDataOwner->Land_SizeNgan) }}  
								{{ (empty(@$contasset->IndenAssetToDataOwner->Land_SizeSquareWa) ? '-' : @$contasset->IndenAssetToDataOwner->Land_SizeSquareWa) }}  
								(ไร-งาน-ตรว.)
							</td>
							<td><b>อำเภอ : </b></td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Land_District }}</td>
							<td><b>จังหวัด : </b></td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Land_Province }}</td>
						</tr>
						<tr>
				...
				@else
					<table border="0">
						<tr>
							<td width="11%"><b>ประเภท :</b></td>
							<td>{{ @$contasset->IndenAssetToDataOwner->AssetToCarType->nametype_car }}</td>
							<td width="11%"><b>ยี่ห้อ : </b></td>
							<td width="15%">{{ Str::limit(@$contasset->IndenAssetToDataOwner->AssetToCarBrand->Brand_car, 30) }}</td>
							<td width="11%"><b>กลุ่ม : </b></td>
							<td>{{ @$contasset->IndenAssetToDataOwner->AssetToCarGroup->Group_car }}</td>
						</tr>
						<tr style="line-height: 90%;">
							<td width="11%"><b>ปี : </b></td>
							<td>{{ @$contasset->IndenAssetToDataOwner->AssetToCarYear->Year_car }}</td>
							<td width="11%"><b>รุ่น : </b></td>
							<td colspan="3">{{ Str::limit(@$contasset->IndenAssetToDataOwner->AssetToCarModel->Model_car, 40) }}</td>
						</tr>
						<tr style="line-height: 90%;">
							<td width="11%"><b>เกียร์ : </b></td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Vehicle_Gear }}</td>
							<td width="11%"><b>สี/ขนาด : </b></td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Vehicle_Color . ' - ' . @$contasset->IndenAssetToDataOwner->Vehicle_CC }}</td>
					...
					<table border="0">
						<tr>
							...
							<td width="34%">{{ @$contasset->IndenAssetToDataOwner->Vehicle_Chassis }}</td>
							<td width="15%"><b>เลขเครื่อง : </b></td>
							<td>{{ @$contasset->IndenAssetToDataOwner->Vehicle_Engine }}</td>
						</tr>
						<tr style="line-height: 90%;">
							<td width="12%"><b>ชำระโดย : </b></td>
							<td>{{ @$CHQMas->CHMasPAYTYP->PAYDESC }}</td>
