    ...
    <div class="table-responsive fw-semibold ">
        <table class="table table-hover">
            <tbody>
                ...
                {{-- Check Credo --}}
                    @php
                         if(@$Credo_Score >= $item->CredoScore || @$tags->Credo_Score >= $item->CredoScore){
                            $credo = @$item->Credo;
                         } else {
                            $credo = 0;
                         }
                ...
                <tr OcuStart = "{{ $item->OcuStart }}" OcuEnd = "{{ $item->OcuEnd }}">
                    @if($item->LTV == 0)
                        <th> <small class="text-success">{{ Auth::user()->position == 'MANAGER' || Auth::user()->position == 'Admin' ?  '( '.$item->LTV + $credo.'% )'  : ''}}</small>  เรท {{ $item->OcuStart }} {{ $item->OcuEnd }}</th>
