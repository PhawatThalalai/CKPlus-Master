...
@php
    ...
    function CalPA($dataPA, $Installment, $totalPeriod)
    {
        ...
        $Data = ['InstallmentPA' => $InstallmentPA, 'Plan' => $Plan, 'textPlan' => $textPlan, 'Limit_Insur' => $Limit_Insur ,'PlanId' => $num_plan];
        return $Data;
    }
@endphp
                    ...
                    @php
                        ...
                        if (in_array($item->Type_Leasing, $arrLeasing) != true) {
                            ...
                            if (@$Type_Customer == 'CUS-0004') {
                                // ถ้าลูกค้าเป็นรีไฟแนนซ์ปิดธนาคาร ค่าดำเนินการจะเป็น 2.3 %
                                ...
                                $arrMicro = ['11', '17','12','13'];
                                if(in_array($item->Type_Leasing, $arrMicro) == true){ // ค่า ดนก ไมโคร
                                    $Flag_operate_fee = @$item->MinOperation_Fee ;
                                }
                            }

                            if(strtolower($statusOPR) == 'nothas'){
                            ...
                            $periodInclude = ((($Cash_Car + $operate_feeInclude) * ($item->Interest / 100) * $item->Installment + ($Cash_Car + $operate_feeInclude)) / $item->Installment) * $Vat; // ค่างวดปกติไม่มี PA
                            $totalPeriodInclude = ($Cash_Car + $operate_feeInclude) * ($item->Interest / 100) * $item->Installment + ($Cash_Car + $operate_feeInclude); // ยอดผ่อนรวม
                            ...
                            $Period_PAInclude = ((($Cash_Car + $InstallmentPAInclude + $operate_feeInclude) * ($item->Interest / 100) * $item->Installment + ($Cash_Car + $InstallmentPAInclude + $operate_feeInclude)) / $item->Installment) * $Vat; // ค่างวดมี PA
                            $totalIntInclude = $totalPeriodInclude - ($Cash_Car + $InstallmentPAInclude + $operate_feeInclude); // ดอกเบี้ย

                            // ไม่รวมค่าดำเนินการ
                            $operate_fee = 0; // ค่าดำเนินการ
                            $period = (($Cash_Car * ($item->Interest / 100) * $item->Installment + ($Cash_Car + $operate_fee)) / $item->Installment) * $Vat; // ค่างวดปกติไม่มี PA
                            $totalPeriod = ($Cash_Car + $operate_fee) * ($item->Interest / 100) * $item->Installment + ($Cash_Car + $operate_fee); // ยอดผ่อนรวม
                            ...
                            $Period_PA = ((($Cash_Car + $InstallmentPA + $operate_fee) * ($item->Interest / 100) * $item->Installment + ($Cash_Car + $InstallmentPA + $operate_fee)) / $item->Installment) * $Vat; // ค่างวดมี PA
                            ...
                            $Period_PAInclude = ($totalIntInclude - ($Cash_Car + $InstallmentPAInclude + $operate_feeInclude)) / $item->Installment;

                            // ไม่รวมค่าดำเนินการ
                            $operate_fee = 0; // ค่าดำเนินการ
                            $period = ((($Cash_Car + $operate_fee) * ($item->Interest / 100) * $item->Installment + ($Cash_Car + $operate_fee)) / $item->Installment) * $Vat; // ค่างวด
                        ...
                        $VAL_IntPeriod_com = (($Cash_Car * floatval($item->Interest)) / 100) * intval($Installment_com); //ดอกเบั้ยรวม
                        ...
                        if ($item->Type_Leasing == 04 || $item->Type_Leasing == 15 || $item->Type_Leasing == 16 || $item->Type_Leasing == 18) {
                            // เช็คค่าแนะนำที่ดิน
                            ...
                            $Commission = floor((((floatval($item->Interest) * $Installment_com) / 100) * intval($Cash_Car + $topupPA) * (intval($item->Commission) / 100)) / 100) * 100;
                            if ($Commission >= 3000) {
                                // ค่าคอมมิชชั่นต้องไม่มากกว่า 3000
                                $Commission = 3000;
                            }
                            ...
                            $Commission = round((intval($Total_IntPeriod_com) * (intval($item->Commission) / 100)) / 100) * 100; //ค่าแนะนำ
                            if ($Commission >= 3000) {
                                // ค่าคอมมิชชั่นต้องไม่มากกว่า 3000
                                $Commission = 3000;
                            }
                        ...
                        array_push($dataArr, [
                            'AddOPR' => [
                                ...
                                'totalPeriod_HasPA' => ceil(@$Period_PAInclude) * $Installment,
                                'totalPeriod_NonPA' => ceil($periodInclude) * $Installment,
                            ...
                            'NonOPR' => [
                                ...
                                'Commission' => $Commission,
                                'totalPeriod_HasPA' => ceil(@$Period_PA) * $Installment,
                                'totalPeriod_NonPA' => ceil($period) * $Installment,
                                'PlanId' => $CalPAInOPR['PlanId'],
                            ],
                            'NotOPR' => [
                                'Cash_Car' => $Cash_Car,
