@foreach (@$data['data-arr'] as $item)
    @if ($item['width'] == 'full')
        ...
        <label class="card-radio-label card__radio__lable__full">
        <input type="radio" name="{{ $item['radio-name'] }}" class="btn-check card-radio-input card-input" value="{{ @$item['btn-value'] }}" autocomplete="off">
        <div class="radio__card_full  btn btn-outline-{{ $item['color'] != '' ? $item['color']  : 'success' }} waves-effect waves-light">
            <div class="radio__card__el__full">
                @if ($iconRes == true)
                <lord-icon
                    id="icon"
                    src="{{ @$item['icon-url'] }}"
