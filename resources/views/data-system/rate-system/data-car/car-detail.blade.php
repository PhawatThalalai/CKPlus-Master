                ...
                <thead class="sticky-top">
                    <tr class="table-dark">
                        <th colspan="5" class="rounded-start rounded-end">
                            ...
                            @if(@count($data) > 0)
                                <!-- <button type="button" class="text-white float-end btn btn-sm btn-warning rounded-pill me-1 Modal-lg" data-bs-toggle="modal" data-bs-target="#Modal-lg" data-link="{{ route('CarRate.edit',0) }}?edit={{'yearcar'}}&BrandID={{@$brand_id}}&GroupID={{@$group_id}}&RateID={{@$rate_id}}&ModelID={{@$model_id}}&Modelname={{str_replace(' ','',@$model_name)}}" style="cursor:pointer;">
                                ...
                                <div class="btn-group float-end">
                                    ...
                                    <div class="dropdown-menu" style="">
                                        <a class="dropdown-item Modal-lg" href="#" data-bs-toggle="modal" data-bs-target="#Modal-lg" data-link="{{ route('CarRate.edit',0) }}?edit={{'yearcar'}}&BrandID={{@$brand_id}}&GroupID={{@$group_id}}&RateID={{@$rate_id}}&ModelID={{@$model_id}}&Modelname={{str_replace(' ','',@$model_name)}}" style="cursor:pointer;">
                                            <i class="bx bx-edit-alt"></i> แก้ไขข้อมูลแบบกลุ่ม
                                        </a>
                                        <div class="dropdown-divider"></div>
                                        <a class="dropdown-item Modal-lg" href="#" data-bs-toggle="modal" data-bs-target="#Modal-lg" data-link="{{ route('CarRate.edit',0) }}?edit={{'yearprice'}}&BrandID={{@$brand_id}}&GroupID={{@$group_id}}&RateID={{@$rate_id}}&ModelID={{@$model_id}}&Modelname={{str_replace(' ','',@$model_name)}}" style="cursor:pointer;">
                                            <i class="bx bx-directions"></i> ปรับราคาแบบกลุ่ม
                                        </a>
                                    </div>
                                </div>
                            @endif
                            <button type="button" class="text-white float-end btn btn-sm btn-success rounded-pill me-1 Modal-lg" data-bs-toggle="modal" data-bs-target="#Modal-lg" data-link="{{ route('CarRate.create') }}?create={{'yearcar'}}&BrandID={{@$brand_id}}&GroupID={{@$group_id}}&RateID={{@$rate_id}}&ModelID={{@$model_id}}&Modelname={{str_replace(' ','',@$model_name)}}" style="cursor:pointer;">
                        ...
                        @foreach($data as $key => $value)
                            <tr style="cursor:grab;">
                                ...
                                <td style="width: 15%;" class="btn_detail active-detail-{{$key+1}}" id="detail-{{$key+1}}" data-id="{{@$value->id}}">
                                    <h5 class="font-size-12 m-0">{{@$value->yearCartype->nametype_car}}</h5>
                                </td>
                                <td style="width: 15%;" class="btn_detail active-detail-{{$key+1}}" id="detail-{{$key+1}}" data-id="{{@$value->id}}">
                                    <h5 class="font-size-12 m-0">{{@$value->Year_car}}</h5>
                                </td>
                                <td style="width: 30%;" class="btn_detail active-detail-{{$key+1}}" id="detail-{{$key+1}}" data-id="{{@$value->id}}">
                                    <h5 class="font-size-12">AT : {{number_format(@$value->PriceAT_car)}}</h5>
                                    <h5 class="font-size-12 text-muted mb-0">MT : {{number_format(@$value->PriceMT_car)}}</h5>
                                </td>
                                <td style="width: 30%;" class="btn_detail active-detail-{{$key+1}}" id="detail-{{$key+1}}" data-id="{{@$value->id}}">
                                    <ul class="text-end list-inline font-size-20 contact-links mb-0">
                                        <li class="list-inline-item">
                                            <a class="dropdown-item edit-details Modal-lg" data-bs-toggle="modal" data-bs-target="#Modal-lg" data-link="{{ route('CarRate.edit',@$value->id) }}?edit={{'pricecar'}}" style="cursor:pointer;">
                                                <i class="{{(@$value->Status_year == 'N')?'mdi mdi-eye-off-outline':'bx bx-edit-alt'}} text-warning"></i>
                                            </a>
                                        </li>
                                        <li class="list-inline-item">
                                            <a class="dropdown-item del-Details" data-id="{{@$value->id}}" data-name="{{@$value->Year_car}}" data-brand="{{@$value->Brand_id}}" data-group="{{@$value->Group_id}}" data-model="{{@$value->Model_id}}" data-rate="{{@$value->Ratetype_id}}" data-dm="{{@$model_name}}" style="cursor:pointer;">
    ...
    $(document).on('click', '.del-Details', function(e) {
        ...
        .then( (value) => {
            if (value.isConfirmed) { // กด OK 
                var del = 'cardetail';
                var _url = "{{ route('CarRate.destroy', ':id' ) }}";
