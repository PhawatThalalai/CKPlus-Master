                        ...
                        <thead class="sticky-top">
                            <tr class="table-dark">
                                <th colspan="3" class="rounded-start rounded-end">
                                    ...
                                    <button type="button" class="text-white float-end btn btn-sm btn-success rounded-pill Modal-lg" data-bs-toggle="modal" data-bs-target="#Modal-lg" data-link="{{ route('CarRate.create') }}?create={{'group'}}&brand_id={{@$brand_id}}&brand_name={{@$brand_name}}" style="cursor:pointer;">
                                        ...
                                        <td class="btn_action active-group-{{$key+1}}" id="group-{{$key+1}}" data-id="{{@$value->id}}" data-brand="{{@$value->Brand_id}}" style="width: 30%;">
                                            <ul class="text-end list-inline font-size-20 contact-links mb-0">
                                                <li class="list-inline-item">
                                                    <a class="dropdown-item edit-Group Modal-lg" data-bs-toggle="modal" data-bs-target="#Modal-lg" data-link="{{ route('CarRate.edit',@$value->id) }}?edit={{'groupcar'}}" style="cursor:pointer;">
                                                        <i class="{{(@$value->Status_group == 'yes')?'mdi mdi-eye-off-outline':'bx bx-edit-alt'}} text-warning"></i>
                                                    </a>
                                                </li>
                                                <li class="list-inline-item">
                                                    <a class="dropdown-item del-Group" data-id="{{@$value->id}}" data-name="{{@$value->Group_car}}" data-brand="{{@$value->Brand_id}}" data-bm="{{@$brand_name}}" style="cursor:pointer;">
		...
		$(".btn_group").on('click', function() {
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    $('#viewModelCar').html(response.html).show('slow');
                }
            });
