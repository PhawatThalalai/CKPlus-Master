                ...
                @foreach ($data as $key => $moto)
                    <div class="slide-content">
                        <div class="nav flex-column nav-pills mb-2" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                            ...
                            <button class="btn btn-soft-info waves-effect waves-light d-flex mb-1 rounded-pill BrandClick active-tabs-{{$key+1}}" id="tabs-{{$key+1}}" data-id="{{$moto->id}}" data-bs-toggle="pill">
                                <i class="mdi mdi-car-wash font-size-14 text-danger me-2"></i> {{$moto->Brand_moto}}
                                <span class="col text-end"> <span title="{{$moto->Brand_moto}}"></span> </span>
                            </button>
...
@elseif(@$scale == 'large')
    <div class="d-grid gap-1 scroller mt-n2">
        <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
            {{csrf_field()}}
            @foreach ($data as $key => $moto)
                <button class="btn btn-soft-info waves-effect waves-light mb-1 rounded-pill BrandClick active-tabs-{{$key+1}} d-flex bd-highlight" id="tabs-{{$key+1}}" data-id="{{$moto->id}}" data-bs-toggle="pill">
                    <span class="me-auto bd-highlight">
                        <i class="mdi mdi-alpha-c-circle-outline font-size-14 text-danger me-2"></i> 
                        {{$moto->Brand_moto}}
                    </span>
                    <!-- <a class="Modal-lg bd-highlight d-none edit-{{$key+1}}" data-bs-toggle="modal" data-bs-target="#Modal-lg" data-link="{{ route('CarRate.edit',@$moto->id) }}?edit={{'brandcar'}}" style="cursor:pointer;">
                        <i class="bx bx-edit-alt text-warning"></i>
                    </a> -->
                    <!-- <a class="Modal-lg bd-highlight d-none edit-tabs-{{$key+1}}" data-bs-toggle="modal" data-bs-target="#Modal-lg" data-link="{{ route('CarRate.edit',@$moto->id) }}?edit={{'brandcar'}}" style="cursor:pointer;">
                    ...
                    <tr>
                        <th class="p-2">
                            <div class="d-flex justify-content-between">
                                <a href="#" class="text-dark BrandClick active-tabs-{{$key+1}}" id="tabs-{{$key+1}}" data-id="{{$moto->id}}" data-bs-toggle="pill">
                                    <i class="mdi mdi-car-wash text-danger me-1"></i> 
                                    <span style="line-height:120%;">{{$moto->Brand_moto}}</span>
                                </a>
                                <a class="edit-brand Modal-lg" data-bs-toggle="modal" data-bs-target="#Modal-lg" data-link="{{ route('CarRate.edit',@$moto->id) }}?edit={{'brandcar'}}" style="cursor:pointer;">
                                    <i class="bx bx-edit-alt text-warning"></i>
                                </a>
                            </div>
                        </th>
