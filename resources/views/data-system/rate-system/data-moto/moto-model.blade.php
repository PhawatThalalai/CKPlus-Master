                ...
                <thead>
                    <tr class="table-dark">
                        <th colspan="3" class="rounded-start rounded-end">
                            ...
                            <button type="button" class="text-white float-end btn btn-sm btn-success rounded-pill Modal-lg" data-bs-toggle="modal" data-bs-target="#Modal-lg" data-link="{{ route('MotoRate.create') }}?create={{'model'}}&ModelID={{@$data[0]->id}}&GroupID={{@$group_id}}&BrandID={{@$brand_id}}" style="cursor:pointer;">
                    ...
                    @foreach($data as $key => $value)
                        <tr style="cursor:grab;" class="{{(@$value->Status_model == 'yes')?'text-danger':''}}">
                            ...
                            <td class="btn_model active-model-{{$key+1}}" id="model-{{$key+1}}" data-id="{{@$value->id}}" data-brand="{{@$value->Brand_id}}" data-group="{{@$value->Group_id}}" data-rate="{{@$value->Ratetype_id}}" style="width: 60%;">
                                <h5 class="font-size-12 m-0">{{@$value->Model_moto}}</h5>
                            ...
                            <td class="btn_option active-model-{{$key+1}}" id="model-{{$key+1}}" data-id="{{@$value->id}}" style="width: 30%;">
                                <ul class="text-end list-inline font-size-20 contact-links mb-0">
                                    <li class="list-inline-item">
                                        <a class="dropdown-item edit-details Modal-lg" data-bs-toggle="modal" data-bs-target="#Modal-lg" data-link="{{ route('MotoRate.edit',@$value->id) }}?edit={{'modelcar'}}" style="cursor:pointer;">
                                            <i class="{{(@$value->Status_model == 'yes')?'mdi mdi-eye-off-outline':'bx bx-edit-alt'}} text-warning"></i>
                                        </a>
                                    </li>
                                    <li class="list-inline-item">
                                        <a class="dropdown-item del-Model" data-id="{{@$value->id}}" data-name="{{@$value->Model_moto}}" data-brand="{{@$value->Brand_id}}" data-group="{{@$value->Group_id}}" data-gm="{{@$group_name}}" style="cursor:pointer;">
        ...
        $(".btn_model").on('click', function() {
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    $('#viewDetailMoto').html(response.html).show('slow');
                }
            });
