!(function (a) {
    ...
    a.sessionTimeout = function (b) {
        ...
        function d() {
            clearTimeout(g),
                (i.countdownMessage || i.countdownBar) &&
                    f("session", true, i.redirAfter - i.warnAfter),
                "function" == typeof i.onStart && i.onStart(i),
                i.keepAlive && c(),
                (g = setTimeout(function () {
                    "function" != typeof i.onWarn
        ...
        function e() {
            clearTimeout(g),
                a("#session-timeout-dialog").hasClass("in") ||
                    (!i.countdownMessage && !i.countdownBar) ||
                    f("dialog", true, i.redirAfter - i.warnAfter),
                (g = setTimeout(function () {
            ...
            //             ? (window.location = i.redirUrl)
            //             : i.onRedir(i);
            //     }, 360000)); // เปลี่ยนจาก i.redirAfter - i.warnAfter เป็นเวลาที่ต้องการให้แสดง modal
        }
        function f(b, c, timeLeft) {
            ...
            var seconds = e % 60;
            var displayTime = minutes + " นาที " + seconds + " วินาที.";
            d.text(displayTime);
            i.countdownBar &&
                a(".countdown-bar").css("width", j.percentLeft + "%"),
                (j.timeLeft = j.timeLeft - 1),
        ...
        var g,
            h = {
                title: "แจ้งเตือนจากงานระบบ !",
                message:
                    "เนื่องจากไม่มีการตอบสนองใดๆกับระบบ คุณจะถูกพาไปยังหน้า lock-screen.",
                logoutButton: "ออกจากระบบ",
                keepAliveButton: "อยู่หน้านี้",
                keepAliveUrl: "/keep-alive",
                ajaxType: "", // ใช้เป็น POST แทน
                ajaxData: "",
                redirUrl: "/timed-out",
                logoutUrl: "/log-out",
                warnAfter: 9e5,
        ...
        // Define the path to the GIF image
        var gifImage =
            '<img src="/assets/images/gif/lock-screen-re.gif" style="width: 80%; ">';
        ...
        if ("function" != typeof i.onWarn) {
            ...
            a("body").append(
                '<div class="modal fade" id="session-timeout-dialog">               <div class="modal-dialog">                 <div class="modal-content">                   <div class="modal-header">                     <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>                     <h4 class="modal-title">' +
                    '<div class="d-flex align-items-center"><i class="bx bx-error-circle text-danger me-2"></i>' +
                    i.title +
                    '</div></h4>                   </div>                   <div class="modal-body">' +
                    '<div class="text-center">' +
                    gifImage + // Add GIF in modal body
                    "</div>" +
                    "<p>" +
                    i.message +
                    "</p>                     " +
