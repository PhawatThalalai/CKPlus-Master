...
class Config_RunHP_BillTmp extends Model
{
    protected $table = 'Config_RunHP_BillTmp';
    protected $fillable = ['Flag_Tags','Code_Tags','Date_Tags','Zone_Tags','UserAdd_Tags'];

    public static function generateJobnumber(){
        $Job = Config_RunHP_BillTmp::whereYear('Date_Tags',date('Y'))->where('Zone_Tags',auth()->user()->zone)->latest('id')->first();
        if($Job != NULL){
    ...
    public static function createJobnumber(){
        ...
        $RunJob = new Config_RunHP_BillTmp([
            'Flag_Tags' => 'Y',
            'Code_Tags' => $CodeJob,
